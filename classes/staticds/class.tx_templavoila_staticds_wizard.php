...
class tx_templavoila_staticds_wizard {
	...
	public function staticDsWizard() {
		...
		$title = $GLOBALS['LANG']->sL('LLL:EXT:templavoila/Resources/Private/Language/template_conf.xml:staticDS.wizard.title.' . $this->step);
		$description = $GLOBALS['LANG']->sL('LLL:EXT:templavoila/Resources/Private/Language/template_conf.xml:staticDS.wizard.description.' . $this->step);
		...
		switch ($this->step) {
			case 1:
				...
				if (t3lib_div::_GP('dsWizardDoIt')) {
					...
					if (!isset($conf['staticDS.']['path_fce']) || !strlen($conf['staticDS.']['path_fce'])) {
						$ok[0] = FALSE;
						$description .= sprintf('||' . $GLOBALS['LANG']->sL('LLL:EXT:templavoila/Resources/Private/Language/template_conf.xml:staticDS.wizard.dircheck.notset'), 'staticDS.path_fce');
					} else {
						$ok[0] = $this->checkDirectory($conf['staticDS.']['path_fce']);
						if ($ok[0]) {
							$description .= sprintf('||' . $GLOBALS['LANG']->sL('LLL:EXT:templavoila/Resources/Private/Language/template_conf.xml:staticDS.wizard.dircheck.ok'), htmlspecialchars($conf['staticDS.']['path_fce']));
						} else {
							$description .= sprintf('||' . $GLOBALS['LANG']->sL('LLL:EXT:templavoila/Resources/Private/Language/template_conf.xml:staticDS.wizard.dircheck.notok'), htmlspecialchars($conf['staticDS.']['path_fce']));
						}
					}

					if (!isset($conf['staticDS.']['path_page']) || !strlen($conf['staticDS.']['path_page'])) {
						$ok[0] = FALSE;
						$description .= sprintf('||' . $GLOBALS['LANG']->sL('LLL:EXT:templavoila/Resources/Private/Language/template_conf.xml:staticDS.wizard.dircheck.notset'), 'staticDS.path_page');
					} else {
						$ok[1] = $this->checkDirectory($conf['staticDS.']['path_page']);
						if ($ok[1]) {
							$description .= sprintf('|' . $GLOBALS['LANG']->sL('LLL:EXT:templavoila/Resources/Private/Language/template_conf.xml:staticDS.wizard.dircheck.ok'), htmlspecialchars($conf['staticDS.']['path_page']));
						} else {
							$description .= sprintf('|' . $GLOBALS['LANG']->sL('LLL:EXT:templavoila/Resources/Private/Language/template_conf.xml:staticDS.wizard.dircheck.notok'), htmlspecialchars($conf['staticDS.']['path_page']));
						}
					}
				...
				if ($ok == array(TRUE, TRUE) && $this->step < 3) {
					$submitText = $conf['staticDS.']['enable']
						? $GLOBALS['LANG']->sL('LLL:EXT:templavoila/Resources/Private/Language/template_conf.xml:staticDS.wizard.submit3')
						: $GLOBALS['LANG']->sL('LLL:EXT:templavoila/Resources/Private/Language/template_conf.xml:staticDS.wizard.submit2');
			...
			default:
				...
				<input type="submit" name="dsWizardDoIt" value="' . $GLOBALS['LANG']->sL('LLL:EXT:templavoila/Resources/Private/Language/template_conf.xml:staticDS.wizard.submit1') . '" />';
				break;
		}
	...
	protected function getDsRecords($conf) {
		...
		foreach ($rows as $row) {
			...
			if (count($writeDsIds) && in_array($row['uid'], $writeDsIds)) {
				...
				if ($options['updateRecords']) {
					...
					$updateMessage = $GLOBALS['LANG']->sL('LLL:EXT:templavoila/Resources/Private/Language/template_conf.xml:staticDS.wizard.updated');
					$this->step = 3;
				}
			}
		...
		if ($conf['enable']) {
			if ($updateMessage) {
				$out .= '<p>' . $updateMessage . '</p><p><strong>' . $GLOBALS['LANG']->sL('LLL:EXT:templavoila/Resources/Private/Language/template_conf.xml:staticDS.wizard.clearcache') . '</strong></p>';
			} else {
				$out .= '<h4>' . $GLOBALS['LANG']->sL('LLL:EXT:templavoila/Resources/Private/Language/template_conf.xml:staticDS.wizard.description2.1') . '</h4>';
				$out .= '<p>
				<input type="checkbox" class="checkbox" name="staticDSwizardoptions[updateRecords]" id="sdw-updateRecords" value="1" />
				<label for="sdw-updateRecords">' . $GLOBALS['LANG']->sL('LLL:EXT:templavoila/Resources/Private/Language/template_conf.xml:staticDS.wizard.updaterecords') . '</label><br />
				</p>';
			}
		}
